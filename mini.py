...
llm = ChatGoogleGenerativeAI(
    model="gemini-2.0-flash-lite", temperature=0.4, google_api_key=GEMINI_API_KEY
)
...
def get_location():
    ...
    try:
        ...
        if not body or "lat" not in body or "lon" not in body:
            return (
                jsonify({"error": "Please provide both 'lat' and 'lon' fields."}),
                400,
            )
        ...
        if response.status_code != 200 or "flowSegmentData" not in data:
            return (
                jsonify({"error": "Failed to fetch traffic data", "details": data}),
                500,
            )
...
def home():
    return {
        "message": "Welcome to the Traffic Summary API! POST to /get_loc with lat/lon."
    }
